...
async function sendTabs(request) {
    const [tabs, { keep_moved_tabs_selected, discard_minimized_window, send_tab_to_start }] = await Promise.all([
        request.tabs ?? getSelectedTabs(), // If tabs not given in request, get selected tabs
        Storage.getDict(['keep_moved_tabs_selected', 'discard_minimized_window', 'send_tab_to_start']),
    ]);
    request.tabs ??= tabs;
    request.keep_moved_tabs_selected = keep_moved_tabs_selected;
    request.send_tab_to_start = send_tab_to_start;
 ...
 * @returns {Promise<Tab[]>}
 */
async function moveTabs({ tabs, windowId, keep_moved_tabs_selected, send_tab_to_start }) {
    ...
    /** @type {Tab[]} */ const movedTabs = (await Promise.all([
        ...
        browser.tabs.move(unpinnedTabs.map(tab => tab.id), { windowId, index: send_tab_to_start ? 0 : -1 }),
    ])).flat();
...
/**
 * @param {Tab[]} tabs
 * @returns {[pinned: Tab[], unpinned: Tab[]]}
 */
